...
[^22-inference-many-means-3]: Let $\bar{x}$ represent the mean of outcomes across all groups.
    Then the mean square between groups is computed as $MSG = \frac{1}{df_{G}}SSG = \frac{1}{k-1}\sum_{j=1}^{k} n_{j} \left(\bar{x}_{j} - \bar{x}\right)^2$ where $SSG$ is called the **sum of squares between groups** $(SSG)$\index{sum of squares between groups} and $n_{j}$ is the sample size of group $j.$
...
[^22-inference-many-means-4]: See [additional details on ANOVA calculations](https://www.openintro.org/download.php?file=stat_extra_anova_calculations) for interested readers.
    ...
    Then the **sum of squares total** $(SST)$\index{sum of squares total} is computed as $$SST = \sum_{i=1}^{n} \left(x_{i} - \bar{x}\right)^2$$ where the sum is over all observations in the dataset.
    Then we compute the **sum of squared errors** $(SSE)$\index{sum of squared errors} in one of two equivalent ways: $SSE = SST - SSG = (n_1-1)s_1^2 + (n_2-1)s_2^2 + \cdots + (n_k-1)s_k^2$ where $s_j^2$ is the sample variance (square of the standard deviation) of the residuals in group $j.$ Then the $MSE$ is the standardized form of $SSE: MSE = \frac{1}{df_{E}}SSE.$
